namespace VideStore.Domain.ErrorHandling
{
    ...
    /// <summary>
    /// Represents an error with an associated HTTP status code and message.
    /// </summary>
    public class Error(int statusCode, string? description = null)
    {
        public static Error None { get; } = new(200, "The request was successful.");

        public int StatusCode { get; } = statusCode;
        public string Description { get; } = !string.IsNullOrWhiteSpace(description) ? description : GetHttpMessage(statusCode);

        private static readonly Dictionary<int, string> HttpMessages = new()
        {
        ...
        /// <summary>
        /// Gets the HTTP message corresponding to the given status code.
        /// </summary>
        public static string GetHttpMessage(int statusCode)
        {
            return HttpMessages.GetValueOrDefault(statusCode, "Unknown Status Code: The status code is not recognized.");
        }
    }
